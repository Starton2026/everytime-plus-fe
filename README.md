...
| 개선한 점 | **게시글에 태그 기능 추가** |
| 이 저장소 | 프론트엔드 — **재윤** 단독 개발 |

백엔드는 팀원 5명이 기능을 나눠 개발했습니다. 역할 분담은
[백엔드 저장소 README](https://github.com/Starton2026/everytime-plus-be#역할-분담)에 정리되어 있습니다.

### 무엇을 왜 개선했나

원래 에브리타임에서 글을 분류하는 수단은 **게시판뿐**입니다.
그래서 "시험 기간에 다른 사람들은 어떻게 하는지" 같은 걸 찾으려면 게시판 목록을 계속 넘겨보거나
검색어가 정확히 맞아떨어지기를 기대해야 합니다.

여기에 **태그**를 더했습니다.

- 글을 쓸 때 `#시험` `#과제` 같은 태그를 최대 3개까지 붙일 수 있습니다.
- 목록에서 태그를 골라 **그 태그가 붙은 글만** 볼 수 있습니다. 여러 개를 고르면 모두 가진 글만 남습니다.
- 검색어와 태그를 **함께** 걸 수 있어서, 같은 단어라도 원하는 맥락의 글만 좁혀집니다.
- 글을 읽다가 태그를 누르면 바로 그 태그의 목록으로 넘어갑니다.

검색은 "정확한 단어를 알고 있을 때" 쓰는 도구지만, 태그는 **단어를 모르더라도 주제로 좁힐 수 있는**
수단이라는 점에서 다릅니다. 이 프로젝트에서 가장 공들인 부분입니다.
...
`.env.example`의 기본 주소가 **배포된 백엔드**를 가리키므로, 그대로 쓰면
[2. 프론트엔드 실행](#2-프론트엔드-실행)만 해도 화면이 동작합니다.
백엔드까지 로컬에서 띄우려면 **백엔드 → 프론트엔드 순서로** 실행하세요.

### 1. 백엔드 실행 (로컬 백엔드를 쓸 때만)
...
`.env.example`의 기본값은 배포된 백엔드 주소입니다.
**로컬 백엔드에 붙이려면** 복사한 `.env.local`의 값을 아래처럼 바꿔주세요.
...
프론트엔드는 **Vercel**, 백엔드는 **Render**에 올라가 있습니다.

- **SPA rewrite**: [vercel.json](vercel.json)에서 모든 경로를 `index.html`로 돌려줍니다.
  이 설정이 없으면 `/boards/1/posts/3` 같은 주소로 직접 접속하거나 새로고침할 때 404가 납니다.
- **API 주소**: 기본값이 이미 배포된 백엔드라 Vercel에 환경 변수를 넣지 않아도 동작합니다.
  다른 백엔드를 쓰려면 Vercel 프로젝트 설정에 `VITE_API_BASE_URL`을 추가하세요.
- **CORS**: 백엔드가 `*.vercel.app`을 허용하도록 되어 있어 프로덕션·미리보기 배포 모두 통과합니다.
  Vercel이 아닌 도메인을 쓰면 백엔드의 `ALLOWED_ORIGINS` 환경 변수에 추가해야 합니다.

> **Render 무료 플랜 주의**
> 일정 시간 요청이 없으면 백엔드가 잠들고, 깨어날 때 파일 시스템이 초기화됩니다.
> 첫 요청이 수십 초 걸릴 수 있고, 그때까지 사용자가 쓴 글은 사라집니다.
> 서버는 기동 시 기본 데이터(게시판 · 테스트 계정 `testuser` / `test1234` · 샘플 글)를
> 자동으로 채우므로, 초기화된 뒤에도 로그인해서 화면을 확인할 수 있습니다.
> 발표·시연 전에 한 번 열어 깨워두는 것을 권합니다.
